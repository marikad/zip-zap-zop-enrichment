...
function add_events_metaoptions() {
	...
	add_meta_box( 
		'pm_header_image_meta', //ID
		 esc_attr__( 'Page Header Image', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_event_date_meta', //ID
		 esc_attr__( 'Event Date', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_event_tooltip_meta', //ID
		 esc_attr__( 'Tooltip Message', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_event_icon_meta', //ID
		 esc_attr__( 'FontAwesome Icon', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_event_countdown_meta', //ID
		 esc_attr__( 'Countdown Timer', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_event_panel_info_meta', //ID
		 esc_attr__( 'Panel Header Info', TEXT_DOMAIN ),  //label
...
function pm_event_date_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Enter the date for this event.',TEXT_DOMAIN); ?></p>
...
function pm_event_tooltip_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Enter a tooltip text.',TEXT_DOMAIN); ?></p>
...
function pm_event_icon_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Enter your own FontAwesome icon for this event post. (ex. fa fa-globe)',TEXT_DOMAIN); ?></p>
...
function pm_event_countdown_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('If you would like to activate the countdown timer for this event please enter the full event date in the following format: 2013,12,25 (year,month,day).',TEXT_DOMAIN); ?></p>
...
function pm_event_panel_info_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Choose between displaying meta information or the event title in the panel header.', TEXT_DOMAIN); ?></p>
        <select id="pm_event_panel_info_meta" name="pm_event_panel_info_meta" class="pm-admin-select-list">  
        	<option value="eventTitle" <?php selected( $pm_event_panel_info_meta, 'eventTitle' ); ?>><?php esc_attr_e('Event Title', TEXT_DOMAIN) ?></option>
            <option value="eventMeta" <?php selected( $pm_event_panel_info_meta, 'eventMeta' ); ?>><?php esc_attr_e('Event Meta', TEXT_DOMAIN) ?></option>
        </select>
    <?php
...
function add_organizer_metaoptions() {
	...
	add_meta_box( 
		'pm_organizer_title_meta', //ID
		 esc_attr__( 'Organizer Title', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_contact_number_meta', //ID
		 esc_attr__( 'Contact Number', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_email_meta', //ID
		 esc_attr__( 'Email Address', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_webiste_meta', //ID
		 esc_attr__( 'Personal Website', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_linkedin_meta', //ID
		 esc_attr__( 'Linkedin Profile', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_twitter_meta', //ID
		 esc_attr__( 'Twitter Profile', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_facebook_meta', //ID
		 esc_attr__( 'Facebook Profile', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_google_meta', //ID
		 esc_attr__( 'Google Plus Profile', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_organizer_skype_meta', //ID
		 esc_attr__( 'Skype Address', TEXT_DOMAIN ),  //label
...
function pm_organizer_title_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Type in the official title of this organizer. (ex. Staff Volunteer)',TEXT_DOMAIN); ?></p>
...
function pm_organizer_tooltip_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Add a tooltip message for this organizer if desired.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_contact_number_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Add a contact number for this organizer if desired.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_email_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Add an email address for this organizer if desired.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_webiste_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Add a web address URL for this organizer if desired.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_linkedin_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Copy and paste in the public Linkedin URL address associated with this organizer - this will activate the Linkedin button.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_twitter_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Copy and paste in the Twitter profile address associated with this staff member - this will activate the Twitter button.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_facebook_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Copy and paste in the Facebook profile address associated with this staff member - this will activate the Facebook button.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_google_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Copy and paste in the Google Plus profile address associated with this staff member - this will activate the Google Plus button.',TEXT_DOMAIN); ?></p>
...
function pm_organizer_skype_meta_function($post) {
	...
	?>
    	<p><?php esc_attr_e('Copy and paste in the Skype member name associated with this staff member - this will activate the Skype button.',TEXT_DOMAIN); ?></p>
...
function add_gallery_metaoptions() {
	...
	add_meta_box( 
		'pm_post_layout_meta', //ID
		 esc_attr__( 'Post Layout', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_gallery_image_meta', //ID
		 esc_attr__( 'Gallery Image', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_gallery_video_meta', //ID
		 esc_attr__( 'Youtube Video', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_gallery_display_video_meta', //ID
		 esc_attr__( 'Display Youtube Video?', TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		'pm_gallery_item_caption_meta', //ID
		 esc_attr__( 'Caption', TEXT_DOMAIN ),  //label
...
function pm_gallery_image_meta_function($post) {
	...
	//HTML code
	?>
    	<p><?php esc_attr_e('Recommended size: 900x400px',TEXT_DOMAIN); ?></p>
		...
		<input id="featured_upload_image_button" type="button" value="<?php esc_attr_e('Media Library Image', TEXT_DOMAIN); ?>" class="button-primary" />
        <div class="pm-admin-gallery-image-preview"></div>
    
    	<?php if($pm_gallery_image_meta) : ?>
        	<input id="remove_gallery_image_button" type="button" value="<?php esc_html_e('Remove Image', TEXT_DOMAIN); ?>" class="button-primary" />
        <?php endif; ?>
...
function pm_gallery_item_caption_meta_function($post) {
	...
	//HTML code
	?>
    	<p><?php esc_attr_e('Enter a caption for your gallery post. This appears in the expanded view.',TEXT_DOMAIN); ?></p>
...
function pm_gallery_video_meta_function($post) {
	...
	//HTML code
	?>
    	<p><?php esc_attr_e('Enter a Youtube video URL (ex. http://www.youtube.com/watch?v=ai9qbTKxwkc)',TEXT_DOMAIN); ?></p>
...
function pm_gallery_display_video_meta_function($post) {
	...
	?>
        <p><?php esc_attr_e('Setting this to YES will override the gallery image in the PrettyPhoto carousel.', TEXT_DOMAIN); ?></p>
        <select id="pm_gallery_display_video_meta" name="pm_gallery_display_video_meta" class="pm-admin-select-list">  
        	<option value="no" <?php selected( $pm_gallery_display_video_meta, 'no' ); ?>><?php esc_attr_e('NO', TEXT_DOMAIN) ?></option>
            <option value="yes" <?php selected( $pm_gallery_display_video_meta, 'yes' ); ?>><?php esc_attr_e('YES', TEXT_DOMAIN) ?></option>
        </select>
...
function pm_ln_custom_sidebar_callback( $post ){
    ...
    $output = '<p><label for="myplugin_new_field">'.esc_html__("Choose a sidebar to display", TEXT_DOMAIN ).'</label></p>';
...
function pm_ln_render_new_post_thumbnail_meta_box() {
    ...
    add_meta_box('postimagediv', esc_html__('Featured Image', TEXT_DOMAIN), 'pm_ln_new_post_thumbnail_meta_box', 'post', 'side', 'low');
	add_meta_box('postimagediv', esc_html__('Featured Image', TEXT_DOMAIN), 'pm_ln_new_organizers_post_thumbnail_meta_box', 'post_organizers', 'side', 'low');	
	add_meta_box('postimagediv', esc_html__('Featured Image', TEXT_DOMAIN), 'pm_ln_new_events_post_thumbnail_meta_box', 'post_events', 'side', 'low');
...
function add_post_metaoptions() {
	...
	add_meta_box( 
		'pm_header_image_meta', //ID
		esc_html__( 'Post Header Image' , TEXT_DOMAIN ),  //label
		'pm_header_image_meta_function' , //function
		'post', //Post type
		'normal', 
		'high' 
	...
	add_meta_box( 
		...
		esc_html__( 'Page Layout', TEXT_DOMAIN ),
	...
	add_meta_box( 
		...
		esc_html__( 'FontAwesome 4 Icon' , TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		...
		esc_html__( 'Tooltip Message' , TEXT_DOMAIN ),  //label
	...
	/*add_meta_box( 
		...
		esc_html__( 'Slider Transition', TEXT_DOMAIN ),
...
function pm_post_slides_function($post) {
	...
	?>
    	...
    	<p><?php _e('Enable Post Slider?', TEXT_DOMAIN) ?></p>
        
        <select id="pm_enable_slider_system" name="pm_enable_slider_system" class="pm-admin-select-list">  
            <option value="no" <?php selected( $pm_enable_slider_system, 'no' ); ?>><?php _e('No', TEXT_DOMAIN) ?></option>
            <option value="yes" <?php selected( $pm_enable_slider_system, 'yes' ); ?>><?php _e('Yes', TEXT_DOMAIN) ?></option>
        </select>
        ...
        <?php } ?>
        	...
        	<p><?php esc_html_e('Select the transition type for the Post slider.', TEXT_DOMAIN); ?></p>
            <select id="pm_post_slide_transition_meta" name="pm_post_slide_transition_meta" class="pm-admin-select-list">  
                <option value="slide" <?php selected( $pm_post_slide_transition_meta, 'slide' ); ?>><?php esc_html_e('Slide', TEXT_DOMAIN) ?></option>
                <option value="fade" <?php selected( $pm_post_slide_transition_meta, 'fade' ); ?>><?php esc_html_e('Fade', TEXT_DOMAIN) ?></option>
            </select>  
                
            <p><?php _e('Add or remove slides. (Recommended size 1170x300px)', TEXT_DOMAIN) ?></p>
                ...
                <?php 
                    ...
                    if(is_array($pm_post_slides)){
                        ...
                        foreach($pm_post_slides as $val) {
                            ...
                            echo '<input type="button" value="'.__('Media Library Image', TEXT_DOMAIN).'" class="button-secondary slider_system_upload_image_button" id="pm_slider_system_post_btn_'.$counter.'" />';
                            echo '&nbsp; <input type="button" value="'.__('Remove Slide', TEXT_DOMAIN).'" class="button button-primary button-large delete slider_system_remove_image_button" id="pm_slider_system_post_remove_btn_'.$counter.'" />';
							echo '</div>';
                        ...
                        echo '<b><i>'.__('No slides found',TEXT_DOMAIN).'</i></b>';
            ...
            <br />
            <input type="button" id="pm-slider-system-add-new-slide-btn" class="button button-primary button-large" value="<?php _e('New Slide',TEXT_DOMAIN) ?>">
...
function pm_post_icon_meta_function($post) {
	...
	//HTML code
	?>    
    	<p><?php esc_html_e('Add a FontAwesome icon for the panel link. (ex. fa fa-globe)', TEXT_DOMAIN); ?></p>
...
function pm_post_tooltip_meta_function($post) {
	...
	//HTML code
	?>    
    	<p><?php esc_html_e('Add a tooltip message if desired.', TEXT_DOMAIN); ?></p>
...
function pm_header_image_meta_function($post) {
	...
	//HTML code
	?>
    	<p><?php esc_html_e('Recommended size: 1920x500px', TEXT_DOMAIN) ?></p>
		...
		<input id="upload_image_button" type="button" value="<?php esc_html_e('Media Library Image', TEXT_DOMAIN); ?>" class="button-primary" />
        <div class="pm-admin-upload-field-preview"></div>
        
        <?php if($pm_header_image_meta) : ?>
        	<input id="remove_page_header_button" type="button" value="<?php esc_html_e('Remove Image', TEXT_DOMAIN); ?>" class="button-primary" />
        <?php endif; ?>        
...
function add_page_metaoptions() {
	...
	add_meta_box( 
		'pm_header_image_meta', //ID
		esc_html__( 'Page Header Image', TEXT_DOMAIN ),
		'pm_header_image_meta_function' , //function
		'page', //Post type
		'normal', 
		'high' 
	...
	add_meta_box( 
		...
		esc_html__( 'Disable Bootstrap container for full width content?', TEXT_DOMAIN ),
	...
	add_meta_box( 
		...
		esc_html__( 'Bootstrap Container Padding Amount' , TEXT_DOMAIN ),  //label
	...
	/*add_meta_box( 
		...
		esc_html__( 'Enable Print and Share options?' , TEXT_DOMAIN ),  //label
	...
	add_meta_box( 
		...
		esc_html__( 'Page Header Message' , TEXT_DOMAIN ),  //label
...
function pm_display_header_meta_function($post) {
	...
	?>
        ...
        <select id="pm_display_header_meta" name="pm_display_header_meta" class="pm-admin-select-list"> 
        	<option value="on" <?php selected( $pm_display_header_meta, 'on' ); ?>><?php esc_html_e('ON', TEXT_DOMAIN) ?></option> 
            <option value="off" <?php selected( $pm_display_header_meta, 'off' ); ?>><?php esc_html_e('OFF', TEXT_DOMAIN) ?></option>
        </select>
