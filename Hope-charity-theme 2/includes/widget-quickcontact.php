...
class pm_quickcontact_widget extends WP_Widget {
	...
	function pm_quickcontact_widget() {
		...
		$widget_ops = array(
			...
			'description' => esc_attr__('Insert a quick contact form',TEXT_DOMAIN)
		);
		
		parent::__construct('pm_quickcontact_widget', esc_attr__('[Micro Themes] - Quick Contact Form',TEXT_DOMAIN), $widget_ops);
	...
	function form($instance){
		...
		?>
        	...
        	<p><?php esc_attr_e( 'Title', TEXT_DOMAIN ) ?> <input class="widefat" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
            <p><?php esc_attr_e( 'Description:', TEXT_DOMAIN ) ?> <input class="widefat" name="<?php echo esc_attr($this->get_field_name('desc')); ?>" type="text" value="<?php echo esc_attr($desc); ?>" /></p>
            <p><?php esc_attr_e( 'Email address:', TEXT_DOMAIN ) ?> <input class="widefat" name="<?php echo esc_attr($this->get_field_name('email')); ?>" type="text" value="<?php echo esc_attr($email); ?>" /></p>
	...
	function widget($args, $instance){
		...
		echo $before_widget;
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_attr__( 'Quick Contact', TEXT_DOMAIN ) : $instance['title'], $instance, $this->id_base );
		...
		<form action="#" method="post" id="quick-contact-form-'.$_SESSION['pm_ln_quick_contact_index'].'" class="validate quick-contact-form" target="_blank" novalidate>  
			<input name="full_name" type="text" class="quick_contact_field" id="pm_full_name" placeholder="'.esc_attr__( 'full name', TEXT_DOMAIN ).'">
			<input name="email_address" type="email" class="quick_contact_field" id="pm_email_address" placeholder="'.esc_attr__( 'email address', TEXT_DOMAIN ).'">
			<textarea name="message" id="pm_message" cols="" rows="" class="quick_contact_textarea" placeholder="'.esc_attr__( 'message', TEXT_DOMAIN ).'"></textarea>
