			  ...
			  echo '<div class="pm-micro-nav-lang-selector '.$class.'">';
				 ...
				 echo '<div class="pm-dropdown pm-language-selector-menu">';
					 echo '<div class="pm-dropmenu">';
						 echo '<p class="pm-menu-title">'.esc_attr__('Language',TEXT_DOMAIN).'</p>';
...
if( !function_exists('pm_hope_page_crumbs') ) {
	...
	function pm_hope_page_crumbs() {
		...
		if ((is_page() && !is_front_page()) || is_home() || is_category() || is_single() || is_tag() || is_404()) {
		 ...
		 echo '<li><a href="'.get_bloginfo('url').'">'. esc_attr__('Home', TEXT_DOMAIN) .'</a></li>';
...
if( !function_exists('pm_hope_mytheme_comment') ) {	
	...
	function pm_hope_mytheme_comment($comment, $args, $depth) {
		...
		<?php printf(esc_attr__('<cite class="fn">%s</cite>', TEXT_DOMAIN), get_comment_author_link()) ?> //
		<a href="<?php echo htmlspecialchars(get_comment_link( $comment->comment_ID )) ?>">
		<?php printf(esc_attr__('%1$s at %2$s', TEXT_DOMAIN), get_comment_date(),get_comment_time()) ?></a><?php edit_comment_link(esc_attr__('(Edit)', TEXT_DOMAIN),' ','') ?>
		</div>
		
		<?php if ($comment->comment_approved == '0') : ?>
		<em style="margin-left:27px;"><?php esc_attr_e('Your comment is awaiting moderation.', TEXT_DOMAIN) ?></em>
		<br />
		<?php endif; ?>
		...
		$comments_args = array(
		  'title_reply'       => esc_attr__( 'Leave a Reply', TEXT_DOMAIN ),
		  'title_reply_to'    => esc_attr__( 'Leave a Reply to %s', TEXT_DOMAIN ),
		  'cancel_reply_link' => esc_attr__( 'Cancel Reply', TEXT_DOMAIN ),
		  'label_submit'      => esc_attr__( 'Post Comment', TEXT_DOMAIN ),
		);
