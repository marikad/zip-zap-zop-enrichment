...
<?php 
	...
	//social icons	
	$globalButtonText = get_theme_mod('globalButtonText', esc_attr__( 'Donate Today', TEXT_DOMAIN ));
            ...
            <div class="row">
                ...
                <?php } ?>                    
                    ...
                    <div class="header_social_icons">   
                        <?php if($facebooklink !== '') { 
							echo '<a href="'.esc_html($facebooklink).'" class="fa fa-facebook pm_tip_static" title="'. esc_attr__('Facebook', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($twitterlink !== '') { 
							echo '<a href="'.esc_html($twitterlink).'" class="fa fa-twitter pm_tip_static" title="'. esc_attr__('Twitter', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($googlelink !== '') { 
							echo '<a href="'.esc_html($googlelink).'" class="fa fa-google-plus pm_tip_static" title="'. esc_attr__('Google Plus', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($linkedinLink !== '') { 
							echo '<a href="'.esc_html($linkedinLink).'" class="fa fa-linkedin pm_tip_static" title="'. esc_attr__('Linkedin', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($vimeolink !== '') { 
							echo '<a href="'.esc_html($vimeolink).'" class="fa fa-vimeo pm_tip_static" title="'. esc_attr__('Vimeo Channel', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
                        <?php if($youtubelink !== '') { 
							echo '<a href="'.esc_html($youtubelink).'" class="fa fa-youtube pm_tip_static" title="'. esc_attr__('YouTube Channel', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($dribbblelink !== '') { 
							echo '<a href="'.esc_html($dribbblelink).'" class="fa fa-dribbble pm_tip_static" title="'. esc_attr__('Dribbble', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($pinterestlink !== '') { 
							echo '<a href="'.esc_html($pinterestlink).'" class="fa fa-pinterest pm_tip_static" title="'. esc_attr__('Pinterest', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($instagramlink !== '') { 
							echo '<a href="'.esc_html($instagramlink).'" class="fa fa-instagram pm_tip_static" title="'. esc_attr__('Instagram', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
						<?php if($flickrlink !== '') { 
							echo '<a href="'.esc_html($flickrlink).'" class="fa fa-flickr pm_tip_static" title="'. esc_attr__('Flickr', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
                        <?php if($skypelink !== '') { 
							echo '<a href="skype:'.esc_attr($skypelink).'?call" class="fa fa-skype pm_tip_static" title="'. esc_attr__('Contact us', TEXT_DOMAIN) .'"></a>';
						} ?>
						<?php if($emailLink !== '') { 
							echo '<a href="mailto:'.esc_attr($emailLink).'" class="fa fa-envelope pm_tip_static" title="'. esc_attr__('Email us', TEXT_DOMAIN) .'"></a>';
						} ?>
						<?php if($rssLink !== '') { 
							echo '<a href="'.esc_html($rssLink).'" class="fa fa-rss pm_tip_static" title="'. esc_attr__('RSS Feed', TEXT_DOMAIN) .'" target="_blank"></a>';
						} ?>
                    </div>
                            ...
                            <div class="pm_searchbar_container">
                                ...
                                <div class="pm_search_field_container searchbar" id="pm_search_field">
                                  <form action="<?php echo home_url( '/' ); ?>" method="get" id="searchform">
                                      <input type="text" name="s" id="s" class="pm_searchfield" maxlength="100" placeholder="<?php esc_attr_e('Type Keywords...', TEXT_DOMAIN) ?>">                                      
                                                ...
                                                echo '<li data-thumb="'.$s['image'].'" class="pmslide_'.$sliderCounter.'"><img src="'.$s['image'].'" alt="img" />';
                                                    ...
                                                    echo '<div class="pm-holder'. ($enableFixedHeight === 'false' ? ' scalable' : '') .'">';
                                                        echo '<div class="pm-caption'. ($enableFixedHeight === 'false' ? ' scalable' : '') .'">';
                                                              ...
                                                              if( !empty($s['title']) ){
                                                                  echo '<h1>'.esc_html__($s['title'], TEXT_DOMAIN).'</h1>';
                                                              }
                                ...
                                <?php if( is_shop() || is_product() || is_product_tag() || is_product_category() ) { ?>
                                 ...
                                 <?php } else { //end of if shopping pages ?>
                                        ...
                                        <?php } ?>
                                            ...
                                            <?php } elseif(is_tag()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Posts tagged with', TEXT_DOMAIN); ?> <?php echo get_query_var('tag'); ?></h2>
                                            <?php } elseif(is_category()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Posts filed in', TEXT_DOMAIN); ?> <?php $cat = get_category( get_query_var( 'cat' ) ); echo $cat->name; ?></h2>
                                            <?php } elseif(is_month()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Archive for', TEXT_DOMAIN); ?> <?php the_time('F, Y'); ?></h2>
                                            <?php } elseif(is_day()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Archive for', TEXT_DOMAIN); ?> <?php the_time('F jS, Y'); ?></h2>
                                            <?php } elseif(is_year()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Archive for', TEXT_DOMAIN); ?> <?php the_time('Y');?></h2>
                                            <?php } elseif(is_author()) { ?>
                                                <h2 class="pm_page_title"><?php _e('Author Archive', TEXT_DOMAIN); ?></h2>
                                            <?php } elseif(is_404()) { ?>
                                                <h2 class="pm_page_title"><?php _e('404 Page Not Found', TEXT_DOMAIN); ?></h2>
                                            <?php } elseif(is_search()) { ?>
                                                <h2 class="pm_page_title"><?php esc_attr_e('Search results for', TEXT_DOMAIN); ?> "<?php echo get_search_query(); ?>"</h2>                                             
