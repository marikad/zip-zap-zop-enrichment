...
class pulsar_video_widgets extends WP_Widget {
	...
	function pulsar_video_widgets() {
		...
		$widget_ops = array('classname' => 'pulsar-videos','description' => esc_attr__('Video Widget - supports Youtube, Vimeo, Dailymotion',TEXT_DOMAIN));
		/* $control_ops = array( 'twitter name' => 'pulsar', 'count' => 3, 'avatar_size' => '32' ); */		
		parent::__construct('pulsar-videos',esc_attr__('[Micro Themes] - Videos',TEXT_DOMAIN),$widget_ops);
...
function form( $instance ) {
		...
		$defaults = array( 'title' => esc_attr__('Videos',TEXT_DOMAIN), 
			'title' => '', 
			'type' => 'Youtube',
			'id' => ''
 			);
        ...
        <p>
        <label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php esc_attr_e('Video Type:', TEXT_DOMAIN) ?></label>
        <select id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" class="widefat">
            <option <?php if ( 'Youtube' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Youtube', TEXT_DOMAIN) ?></option>
            <option <?php if ( 'Vimeo' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Vimeo', TEXT_DOMAIN) ?></option>
            <option <?php if ( 'Dailymotion' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Dailymotion', TEXT_DOMAIN) ?></option>
        </select>
        </p>

		<p>
		<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php esc_attr_e('Video ID:', TEXT_DOMAIN) ?></label>
