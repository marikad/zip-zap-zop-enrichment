...
class pm_recentposts_widget extends WP_Widget {
	...
	function pm_recentposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => esc_attr__('Display recent posts with style.',TEXT_DOMAIN)
		);
		
		parent::__construct('pm_recentposts_widget', esc_attr__('[Micro Themes] - Recent Posts',TEXT_DOMAIN), $widget_ops);
	...
	function form($instance){
		...
		?>
            ...
            <p><?php esc_attr_e('Number of Posts to display:',TEXT_DOMAIN) ?><input class="widefat" name="<?php echo esc_attr($this->get_field_name('numOfPosts')); ?>" type="text" value="<?php echo esc_attr($numOfPosts); ?>" /></p>
			...
			echo '<li>';
				...
				echo '<div class="pm-hover-item pm_recent_posts" style="height:62px;">
						<div class="pm-hover-item-title-panel">
							...
							<p>'. esc_attr__('Posted',TEXT_DOMAIN) .' '. mysql2date('l, F j, Y', $recent["post_date"]) .'</p>
						</div>
						<div class="pm-hover-item-details" style="height:62px;">
							<div class="pm-hover-item-spacer">
								<p>'.pm_hope_string_limit_words($excerpt,10) .'...<a href="'.get_permalink($recent["ID"]).'">'. esc_attr__('Read More &raquo;',TEXT_DOMAIN) .'</a></p>
								<i class="icon-comments"></i> '. $recent["comment_count"] .' '. esc_attr__('comments',TEXT_DOMAIN) .'                                 
							</div>
						</div>
